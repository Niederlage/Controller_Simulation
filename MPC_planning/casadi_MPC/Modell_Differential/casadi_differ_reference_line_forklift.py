...
class CasADi_MPC_differ_forklift:
    def __init__(self):
        self.base = 2.
        self.LF = 3.
        self.LB = 1.
        ...
        self.v_max = 2.
        self.omega_max = ca.pi * 90 / 180
        self.a_max = ca.inf
        self.omega_rate_max = ca.inf
        self.jerk_max = ca.inf
    ...
    def init_objects(self, dt, x, ref_path):
        sum_total_dist = 0
        ...
        obj = 0 * self.wg[3] * sum_total_dist \
              + 0 * self.wg[0] * sum_v \
              + self.wg[4] * sum_omega \
              + self.wg[2] * sum_a \
    ...
    def states_initialization(self, start, reference_path):
        ...
        for i in range(self.horizon):
            ...
            if i > 0:
                ...
                dyaw = self.normalize_angle(reference_path[2, i] - reference_path[2, i - 1])
    ...
    def init_model_reference_line(self, start, reference_path):
        self.horizon = len(reference_path.T)
        self.get_dt(reference_path)
        ...
        F = self.init_objects(dt, x, reference_path)
    ...
    def get_result_reference_line(self):
        ...
        cal_traj = ca.reshape(self.x_opt[startp:], self.nx, self.horizon)
        op_controls = np.array(cal_traj[3:7, :])
        op_trajectories = np.array(cal_traj[:3, :])
        omega_ = op_controls[1, :]
        v_ = op_controls[0, :]
        steer_ = np.arctan2(self.base * omega_ / v_, 1)
        v_d = v_ / np.cos(steer_)
        op_controls = np.vstack((v_d, steer_))
        # op_controls = np.vstack((v_, omega_))
...
def main():
    ...
    cmpc = CasADi_MPC_differ_forklift()
    ...
    op_dt, op_path, op_input = cmpc.get_result_reference_line()

...
if __name__ == '__main__':
    ...
    loadtraj = np.load("../casadi_MPC/saved_traj.npz")
    ...
    diff_s = np.diff(np.array(op_trajectories[:2, :]), axis=1)
    sum_s = np.sum(np.hypot(diff_s[0, :], diff_s[1, :]))
    print("total number for iteration: ", cmpc.horizon)
    ...
    if tracking:
        trajectory = ut.try_tracking(zst, op_controls, trajectory, None, ref_traj=ref_traj)
        print("Done")

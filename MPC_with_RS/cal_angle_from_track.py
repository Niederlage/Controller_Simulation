...
def generate_orientation(track, starter):
    l_track = len(track.T)
    vlast = starter[:2]
    vec_list = []
    th_list = []
    ...
    for i in range(1, l_track):
        vec = get_vector(track[:, i - 1], track[:, i], "+")
        if check_flip(vlast, vec):
            sign = "-"
            # print("flipped!")
        else:
            sign = "+"

        vec_ = get_vector(np.array([0, 0]), vec, sign)
        theta = np.arctan2(vec_[1], vec_[0])
        theta_mod = mod_theta(theta, t_last)
...
vec_cal, theta_cal = generate_orientation(track, start)

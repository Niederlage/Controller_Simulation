...
try:
    ...
    import PythonRobotics.PathPlanning.ReedsSheppPath.reeds_shepp_path_planning as rs
    from car import move, check_car_collision, MAX_STEER, WB, plot_car
except Exception:
    raise

XY_GRID_RESOLUTION = 10  # [m]
...
STEER_CHANGE_COST = 15.0  # steer angle change penalty cost
...
def main():
    ...
    saved_hybrid_a_star_traj = np.array([x, y, yaw]) / XY_GRID_RESOLUTION
    saved_hybrid_a_star_traj[2, :] *= XY_GRID_RESOLUTION
    saved_hybrid_a_star_ob = np.array([ox, oy]) / XY_GRID_RESOLUTION
    np.savez("../saved_hybrid_a_star.npz", saved_traj=saved_hybrid_a_star_traj, saved_ob=saved_hybrid_a_star_ob)
